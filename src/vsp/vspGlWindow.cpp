...
int VirtGlWindow::mousePress( int mx, int my, int sx, int sy )
{
...
void VirtGlWindow::mouseRelease( int mx, int my, int sx, int sy )
{
	if ( !showFlag ) 
		return;
	...
	currTrack.poll( STOP,  mx-wx, my-wy, sx, sy );
}

void VirtGlWindow::mouseDrag( int mx, int my, int sx, int sy )
{
//	mouseMove(mx,my);
	...
	if ( (Fl::event_state(FL_BUTTON1) && Fl::event_button3() ) ||
		...
		currTrack.poll( SCALE,  mx-wx, my-wy, sx, sy );
	else if ( Fl::event_state(FL_BUTTON3 ) ||
                 (Fl::event_state(FL_BUTTON1) && Fl::event_alt()))
		currTrack.poll( TRANSLATE,  mx-wx, my-wy, sx, sy );
	else if ( Fl::event_state(FL_BUTTON1 ) )
		currTrack.poll( ROTATE,  mx-wx, my-wy, sx, sy );

}

void VirtGlWindow::mouseWheel( int x, int y, int sx, int sy )
{

	if ( !showFlag ) 
		return;

	currTrack.poll( TRANSLATE_SCROLL,  x, y, sx, sy );
}

void VirtGlWindow::mouseMove( int mx, int my, int sx, int sy )
{
	mousex = mx % ww;
	mousey = my % wh;
		...
		case FL_PUSH:
			{
				...
				{
					vWin[i]->deactivate();
					if ( vWin[i]->mousePress( mx, my, sx, sy ) )
						currVWin = vWin[i];
		...
		case FL_RELEASE:
			{
				if ( currVWin )
				{
					currVWin->mouseRelease( mx, my, sx, sy );
				}
				redraw();
				return 1;
			}
		...
		case FL_MOVE:
			{
				if ( currVWin )
				{
					currVWin->mouseMove( mx, my, sx, sy );
					if (mouseTrack) 
					{
		...
		case FL_DRAG:
			{
				if ( currVWin )
				{
					currVWin->mouseDrag( mx, my, sx, sy );
				}
				redraw();
				return 1;
			}
		...
		case FL_MOUSEWHEEL:
			{
				if ( currVWin )
				{
					currVWin->mouseWheel( mx, my, sx, sy );
				}
				redraw();
				return 1;
			}
