...
void Ms_wing_geom::write_degenGeomCsv_file(DegenGeom* degenGeom, FILE* file_id)
{
	fprintf(file_id, "\nSURFACE,%s\n", (char*) getName());
	mwing_surf.write_degenGeomCsv_file(degenGeom, file_id);

	if ( sym_code == NO_SYM ) return;

	fprintf(file_id, "\nSURFACE,%s_refl\n", (char*) getName());
	mwing_surf.write_refl_degenGeomCsv_file(degenGeom, file_id);
}

void Ms_wing_geom::write_degenGeomM_file(DegenGeom* degenGeom, FILE* file_id)
{
	fprintf(file_id, "\ndegenGeom(end+1).type = 'SURFACE';");
	fprintf(file_id, "\ndegenGeom(end).name = '%s';", (char*) getName());
	mwing_surf.write_degenGeomM_file(degenGeom, file_id);

	if ( sym_code == NO_SYM ) return;

	fprintf(file_id, "\ndegenGeom(end+1).type = 'SURFACE';");
	fprintf(file_id, "\ndegenGeom(end).name = '%s_refl';", (char*) getName());
	mwing_surf.write_refl_degenGeomM_file(degenGeom, file_id);
}

//==== Create Degenerate Geometry ====//
DegenGeom* Ms_wing_geom::createDegenGeom()
{
